...
from JARVISROBO import BOT_USERNAME, MESSAGE_DUMP, MONGO_DB_URI, app
from JARVISROBO.utils.custom_filters import PREFIX_HANDLER

# <=======================================================================================================>

FILLERS = {}

BOT_OWNER = list({int(x) for x in ("7157587567").split()})

_MGCLIENT: AgnosticClient = AsyncIOMotorClient(MONGO_DB_URI)

_DATABASE: AgnosticDatabase = _MGCLIENT["JARVISROBOAnime"]
...
DOWN_PATH = "JARVISROBO/downloads/"
    ...
    async def wrapper(_, message: Message):
            ...
            try:
                ...
                if nut - out < 1.2:
                    ...
                    if USER_WC[user] == 3:
                        ...
                        await clog("JARVISROBO", f"UserID: {user}", "SPAM")
                    if USER_WC[user] == 5:
                        ...
                        await clog("JARVISROBO", f"UserID: {user}", "BAN")
                        return
        ...
        try:
            ...
            try:
                ...
                await clog("JARVISROBO", e, "FAILURE", msg=message)
        ...
        if user in BOT_OWNER or user == int(cqowner):
            ...
            try:
                ...
                try:
                    ...
                    await clog("JARVISROBO", e, "FAILURE", cq=c_q)
        else:
            if cqowner_is_ch:
                if user_valid:
                    try:
...
example: /anime Sword Art Online"""
    ...
    try:
        ...
        await clog("JARVISROBO", title_img, "LINK", msg=message)
...
example: /manga Sword Art Online"""
    ...
    try:
        ...
        await clog("JARVISROBO", pic, "LINK", msg=message)
        await client.send_photo(gid, failed_pic, caption=finals_, reply_markup=buttons)
...
async def character_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await clog("JARVISROBO", img, "LINK", msg=message)
...
example: /airing Sword Art Online"""
    ...
    try:
        ...
        await clog("JARVISROBO", coverImg, "LINK", msg=message)
...
This material is marked 18+ and not allowed in this group""",
    ...
    try:
        ...
        await clog("JARVISROBO", pic, "LINK", msg=cq)
        await cq.edit_message_media(
...
➠ *JARVISROBO provides you the best anime-based commands including anime news and much more!*
