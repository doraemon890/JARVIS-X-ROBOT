...
from JARVISROBO.plugins.helper_funcs.misc import is_module_loaded
from JARVISROBO.utils.cmdprefix import CMD_STARTERS
...
if is_module_loaded(FILENAME):
    ...
    async def disable_module(
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "JARVISROBO.plugins." + args[0].rsplit(".", 1)[0]
    ...
    async def enable_module(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(args) >= 1:
            enable_module = "JARVISROBO.plugins." + args[0].rsplit(".", 1)[0]
